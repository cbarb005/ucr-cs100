...
-10, 0, 5, 23
 ```
It sorts all the elements between from start to end.
In fact, `sort` goes over all successive arraries from the start address to the end address.
...
##sort other types
`sort` not only applies to an array or arrays, but also vectors or structs. This is an example of vector rewritten from wikipedia:
```
...
Just like the old saying goes: "every coin has two sides". It is up to you to decide which version to use.
